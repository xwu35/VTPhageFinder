if config["assembler"]=="megahit":
    rule megahit_assembly:
        ...
        shell:
            ...
            # only use singleton fastq file if the total number of lines is >= 4 (-ge 4: greater than or equal; 4 lines means 1 read in fastq)
            if [[ $(zcat {input.singleton} | wc -l) -ge 4 ]]; then
                single="-r {input.singleton}"
            else
                single=""
            fi
    ...
    rule spades_single_cell_mode_assembly:
        ...
        params:
            outdir=directory(os.path.join(dir["output"]["assembly"], "intermediate", "{sample}")),
            setting=config["spades_sc"]["setting"]
        threads:
            config["resources"]["med_cpu"]
        resources:
            mem_mb=config["resources"]["med_mem"]
